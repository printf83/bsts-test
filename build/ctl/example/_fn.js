...
exports.replaceExtention = exports.replaceEConsole = exports.getLibBaseOnSource = exports.getCSSBaseOnSource = exports.getRootBaseOnSource = exports.codePen = exports.codeBeautify = exports.console = exports.toast = void 0;
const bsts_1 = require("@printf83/bsts");
const js_beautify_1 = __importDefault(require("js-beautify"));
const js_beautify_2 = require("js-beautify");
const toast = (color, elem, icon) => {
...
exports.toast = toast;
const console = (elem, title, msg, color) => {
    ...
    if (exampleCodeContainer) {
        ...
        exampleCodeContainer.dispatchEvent(new CustomEvent("bs.console.log", {
            detail: {
                title: title,
                msg: msg ? msg : "Message",
                color: color,
...
const codeBeautify = (type, source_text) => {
    switch (type) {
        case "html":
            source_text = source_text.replace(/\>\</g, ">\n<");
            return (0, js_beautify_2.html_beautify)(source_text, {
                preserve_newlines: false,
                end_with_newline: true,
                indent_size: 4,
            });
        case "css":
            return (0, js_beautify_2.css_beautify)(source_text, {
                preserve_newlines: false,
                end_with_newline: true,
                indent_size: 4,
            });
        default:
            source_text = source_text.replace(/\};/g, "};\n");
            return (0, js_beautify_1.default)(source_text, {
...
exports.codeBeautify = codeBeautify;
const codePen = (data) => {
    if (data) {
        ...
        bsts_1.core.appendChild(document.body, new bsts_1.h.form({
            id: `codepen-form-${id}`,
            target: "_blank",
        ...
        const form = document.getElementById(`codepen-form-${id}`);
        form.submit();
...
const attrHasClass = (attr, className) => {
    if (attr.class) {
...
const getRootBaseOnSource = (attrPreview, attrOutput) => {
    var _a;
    if (attrPreview) {
        delete attrPreview.overflow;
        (_a = attrPreview.padding) !== null && _a !== void 0 ? _a : (attrPreview.padding = 4);
        if (attrPreview.padding === 0 || attrPreview.padding === "0") {
            delete attrPreview.padding;
        }
        if (attrOutput) {
            attrOutput.id = "root";
            return bsts_1.core.getHtml(new bsts_1.h.div(attrPreview, new bsts_1.h.div(attrOutput)));
        }
        else {
            return bsts_1.core.getHtml(new bsts_1.h.div(attrPreview, new bsts_1.h.div({ id: "root" })));
        }
    }
    else {
        if (attrOutput) {
            attrOutput.id = "root";
            return bsts_1.core.getHtml(new bsts_1.h.div({ padding: 4 }, new bsts_1.h.div(attrOutput)));
        }
        else {
...
exports.getRootBaseOnSource = getRootBaseOnSource;
const getCSSBaseOnSource = (attr) => {
    if (attr) {
        let result = [];
...
exports.getCSSBaseOnSource = getCSSBaseOnSource;
const getLibBaseOnSourceIsContain = (str, find) => {
    let found = false;
    find.forEach((i) => {
        if (str.indexOf(i) > -1) {
            found = true;
...
const getLibBaseOnSource = (strCode, strManager, strExtention) => {
    let libImported = ["core"];
    const libList = [
        {
            find: [" b.", "(b.", "...b."],
            lib: "b",
...
exports.getLibBaseOnSource = getLibBaseOnSource;
const replaceEConsole = (strCode) => {
    let consoleFn = undefined;
...
exports.replaceEConsole = replaceEConsole;
const replaceExtention = (rule, strCode) => {
    if (strCode && rule && rule.length > 0) {
        rule.forEach((i) => {
            strCode = strCode === null || strCode === void 0 ? void 0 : strCode.replace(new RegExp(i.find, "gm"), i.replace);
        });
    }
    return strCode;
};
exports.replaceExtention = replaceExtention;
