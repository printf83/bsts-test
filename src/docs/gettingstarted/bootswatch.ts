...
export const bootswatch: IContent = {
	...
	db: [
		...
		{
			source: `() => {
                        return [1, 2, 3, 4, 5, 6].map((i) => new h.h(i, [\`h\${i}. Heading\`, new h.small({ textColor: "body-secondary" }, " with faded secondary text")]));
                    }`,
		},
		{
			source: `() => {
		...
		},
		{
			source: `() => {
                        return new b.table.container([
                            new b.table.thead(new b.table.tr([new b.table.th({ scope: "col" }, "Class"), new b.table.th({ scope: "col" }, "Heading"), new b.table.th({ scope: "col" }, "Heading")])),
                            new b.table.tbody([undefined, "primary", "secondary", "success", "danger", "warning", "info", "light", "dark"].map((i) => new b.table.tr({ color: i }, [new b.table.th({ scope: "row" }, i ? core.uppercaseFirst(i) : "Default"), new b.table.td("Cell"), new b.table.td("Cell")]))),
                        ]);
                    }`,
		},
		{
			...
			source: `() => {
                        return new b.list.container(["primary", "secondary", "success", "danger", "warning", "info", "light", "dark"].map((i) => {
                            return new b.list.item({
                                ...
                                justifyContent: "between",
                                alignItem: "center",
                            }, [core.uppercaseFirst(i), new b.badge({ textBgColor: i, rounded: "pill" }, core.rndBetween(1, 20).toString())]);
                        }));
                    }`,
		},
		{
