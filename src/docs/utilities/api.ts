...
import { BOOTSTRAP_MAIN_VERSION } from "../../ctl/main/env.js";
	...
	item: () => {
		return [
				...
				new e.table({
					item: [
						...
						[
							...
							"Optional",
							"{{false}}",
							`Boolean to enable {{https://getbootstrap.com/docs/${BOOTSTRAP_MAIN_VERSION}/getting-started/rfs/::fluid rescaling with RFS}}.`,
						],
						[
			...
			new e.section([
				...
				new e.text(
					"As a map, like with {{nav:docs/utilities/opacity::opacity utilities}}:"
				),
				new e.codepreview({
					type: "css",
					code: `
