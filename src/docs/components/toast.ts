	...
	item: (db?: e.IBsExampleData[]) => {
		...
		return [
					...
					output: () => {
						return [
							...
							new b.toast.container(
								...
								new b.toast.item(
									{ id: "liveToast", live: "assertive", atomic: true },
									[
										new b.toast.header([
											new h.div({
												bgColor: "primary",
			...
			new e.section([
				...
				new e.text(
					"You can use Bootstrap simple toast by calling {{b.toast.create}} function."
				),
				new e.table({
					item: [
					...
					output: () => {
						const item = (msg: string, timer: string) => {
							return new b.toast.item(
								{ debug: true, live: "assertive", atomic: true },
								[
									new b.toast.header([
										new h.div({
											bgColor: "primary",
			...
			new e.section([
				...
				new e.code({
					...
					output: () => {
						return new b.toast.item(
							{ debug: true, live: "assertive", atomic: true, autohide: false },
							[
								new b.toast.header([
									new h.div({
										bgColor: "primary",
...
item("Heads up, toasts will stack automatically", "2 second ago"),
		...
		{
			source: `() => {
return new b.toast.item({ debug: true, alignItem: "center", live: "assertive", atomic: true }, new h.div({ display: "flex" }, [
new b.toast.body("Hello, world! This is a toast message."),
new b.toast.btnclose({ marginEnd: 2, margin: "auto" }),
]));
}`,
...
return new h.div({ aria: { live: "polite", atomic: "true" }, position: "relative" }, new b.toast.container({ debug: true, placement: "top-end" }, [
item("See? just like this", "Just now"),
item("Heads up, toasts will stack automatically", "2 second ago"),
]));
}`,
...
rounded: true,
position: "relative",
}, new h.div({ position: "absolute", top: 50, start: 50, tMiddle: true }, new b.toast.item({
id: "example-toast",
live: "assertive",
atomic: true,
autohide: false,
