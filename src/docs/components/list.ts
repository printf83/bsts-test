...
export const list: IAttrContent = {
	...
	item: [
		...
		new e.text("Bootstrap TS not {{b::use the standard}} {{cb::.btn}}{{b:: classes here}}."),
		new e.code({
			output: () => {
		...
		new e.text(
			"With {{b.list.item}}s or {{b.listDiv.item}}, Bootstrap TS also make use of the {{disabled}} attribute instead of the {{.disabled}} class automaticly."
		),
		...
		new e.text(
			"Fortunately in Bootstrap TS, you only need to set {{toggle:true}} on {{new b.listDiv.item}} to do activate list group navigation"
		),
		...
		new e.text(
			"By default, Bootstrap TS add {{.fade}} class automaticly into {{b.listDiv.tab.pane}} component. If you don't like it, you can disable it by set {{animation:false}} to each {{b.listDiv.tab.pane}}"
		),
		new e.code({
			output: () => {
				let data = ["home", "profile", "messages", "settings"];
