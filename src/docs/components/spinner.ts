	...
	item: (db?: e.ISourceDB[]) => {
		...
		return [
			...
			new e.section([
				...
				new e.code({
					...
					output: () => {
						return new h.div(
							{ clearfix: true },
							new b.spinner({ type: "border", float: "end" })
						);
					},
				}),
			]),
						...
						return [
							new b.button(
								...
								new b.caption(
									{
										iconPosition: "end",
										icon: new b.spinner({ type: "grow", small: true }, ""),
									},
									"Loading"
								)
							),
			...
			new e.section([
				...
				new e.text(
					"Used for creating the CSS animations for Bootstrap spinners. Included in {{scss/_spinners.scss}}."
				),
