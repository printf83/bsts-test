...
export const columns: IContent = {
	...
	item: (db?: e.IBsExampleData[]) => {
		...
		return [
			...
			new e.section([new e.title("Alignment"), new e.text("Use flexbox alignment utilities to vertically and horizontally align columns.")]),
				...
				new e.code({
					...
					output: () => {
						return new h.div(
							...
							new h.div({ row: true }, [new h.div({ col: true, alignSelf: "start" }, "One of three columns"), new h.div({ col: true, alignSelf: "center" }, "One of three columns"), new h.div({ col: true, alignSelf: "end" }, "One of three columns")])
						);
					},
				}),
			]),
			...
			new e.section([
				...
				new e.code({
					...
					output: () => {
						return new h.div(
							{ container: true, textAlign: "center" },
							new h.div({ row: true }, [new h.div({ col: true }, "First in DOM, no order applied"), new h.div({ col: true, order: 5 }, "Second in DOM, with a larger order"), new h.div({ col: true, order: 1 }, "Third in DOM, with an order of 1")])
						);
					},
				}),
				...
				new e.code({
					...
					output: () => {
						return new h.div(
							{ container: true, textAlign: "center" },
							new h.div({ row: true }, [new h.div({ col: true, order: "last" }, "First in DOM, ordered last"), new h.div({ col: true }, "Second in DOM, unordered"), new h.div({ col: true, order: "first" }, "Third in DOM, ordered first")])
						);
					},
				}),
			]),
