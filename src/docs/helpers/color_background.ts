...
"light",
"dark",
].map((i) => new h.div({ textBgColor: i, padding: 3 }, \`\${core.uppercaseFirst(i)} with contrasting color\`));
}`,
		},
		{
			source: `() => {
...
new b.badge({ textBgColor: "info" }, "Info"),
		...
		{
			source: `() => {
const item = (textBgColor) => new b.card.container({ textBgColor: textBgColor, style: { width: "18rem" } }, [
new b.card.header("Header"),
