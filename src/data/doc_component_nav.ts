...
export const doc_component_nav: IAttrContent = {
	...
	item: [
		...
		new e.subtitle("Tabs"),
		new e.text(
			"Takes the basic nav from above and adds the {{.nav-tabs}} class to generate a tabbed interface. Use them to create tabbable regions with our {{nav:doc_component_nav#javascript_behavior::tab JavaScript plugin}}."
		),
		new e.code({
			output: () => {
		...
		new e.text(
			"Add dropdown menus with a little extra HTML and the {{nav:doc_component_dropdown#usage::dropdowns JavaScript plugin}}."
		),
