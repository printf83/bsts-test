...
export const zindex: IAttrContent = {
	...
	item: [
		new e.title("Example"),
		new e.text(
			"Use {{z-index}} utilities to stack elements on top of one another. Requires a {{position}} value other than {{static}}, which can be set with custom styles or using Bootstrap {{nav:doc/utilities/position::position utilities}}."
		),
		new e.alert(
		...
		new e.code({
			...
			output: () => {
				return [3, 2, 1, 0, "n1"].map(
					(i) =>
						new h.div(
							{ zIndex: i as core.IAttr["zIndex"], position: "absolute", rounded: 3, padding: 5 },
							new h.span(`z-${i}`)
						)
				);
			},
