	...
	item: [
		...
		new e.ul({
			item: [
				...
				"As a fallback, {{.is-invalid}} and {{.is-valid}} classes may be used instead of the pseudo-classes for {{nav:doc/form/validation#server_side::server-side validation}}. They do not require a {{.was-validated}} parent class.",
				...
				"Feedback messages may utilize the {{nav:doc/form/validation#browser_defaults::browser defaults}} (different for each browser, and unstylable via CSS) or Bootstrap custom feedback styles with additional HTML and CSS.",
		...
		new e.text(
			"Two mixins are combined, through Bootstrap {{nav:doc/form/validation#loop::loop}}, to generate Bootstrap form validation feedback styles."
		),
		new e.codepreview({
			type: "css",
